...
├─────────────────────────────────────────────────────────────┤
│  [F1:Overview] [F2:Ingest] [F3:Pipeline] [F4:Network] ...   │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│  PIPELINE PROCESSING                     [F3] Page 3/6      │
├─────────────────────────────────────────────────────────────┤
...
### 3.3 Navigation Model (FINAL CORRECTED)

**Primary Navigation (F-keys):**
```
Key Bindings:
  F1          : Overview page (default on startup)
  F2          : Ingest page
  F3          : Pipeline page
  F4          : Network page
  F5          : Events page
...
```
Key Bindings:
  ? or h      : Toggle help overlay (NOT F1)
```
...
**Navigation Contract:**
- F1 is **always** Overview (not Help)
- Help is accessed via `?` or `h` key
- Footer on all pages shows: `[F1:Overview] [F2:Ingest] ... [?:Help]`

---

## 4. Performance Optimization Strategy

### 4.1 Concurrency and Ownership Model (CORRECTED)
...
### 4.2 VirtualList with Snapshot Isolation (CORRECTED)
...
### 4.3 Frame Scheduler with Bounded Drain (CORRECTED)
...
func (f *frameScheduler) run() {
    ...
    for {
        ...
        case <-ticker.C:
            f.flush()
        case <-f.quit:
            // Final flush before exit (bounded by caller timeout)
            f.flush()
            return
        }
    }
...
2. `run()` sees quit, calls final `flush()`, returns
3. `Stop()` waits for `wg.Wait()` with 100ms timeout
4. If timeout: log warning, return (updates may be lost)
5. **No goroutine leak:** `wg.Done()` called in deferred `run()` exit

### 4.4 Search Debouncer with Lifecycle (CORRECTED)
...
### 4.5 Memory-Bounded Event Buffers with Drop Policy (CORRECTED)
...
1. **New dashboard implementation**
   - `ui/dashboard_v2.go`
   - Page switcher
   - Keyboard navigation (F1-F6, ?, alternatives)
...
- Alternative (pooling) adds complexity; measure first, optimize if needed

### 6.3 Cancellation and Shutdown (CORRECTED)
...
### 8.2 New Configuration Options (CORRECTED)
...
- `ui_buffer_utilization` - Event buffer fill ratio (gauge)
- `ui_page_switches` - Navigation counter (counter)
...
| 5 | Agreed | Add keyboard navigation (F1-F6, ?, alternatives) |
...
*Changes from v2: Fixed F1 navigation (Overview, not Help), Help moved to ? key, implemented bounded drain with timeout, defined concrete Snapshot API with ownership/lifetime, added SearchFilter lifecycle with context, added DropPolicy with config, clarified snapshot allocation strategy, added metrics labels for drops*
