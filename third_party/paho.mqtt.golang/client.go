...
type client struct {
	...
	inboundPublishQueueMu          sync.RWMutex
	inboundPublishQueue            chan *packets.PublishPacket
	inboundPublishDropsQoS0        uint64
	inboundPublishQoS12Timeouts    uint64
	inboundPublishQoS12Disconnects uint64
	inboundPublishDropLimiter      inboundPublishLimiter
	inboundPublishQoS12Limiter     inboundPublishLimiter
...
func NewClient(o *ClientOptions) Client {
	...
	c.inboundPublishDropLimiter = newInboundPublishLimiter(inboundPublishLogInterval)
	c.inboundPublishQoS12Limiter = newInboundPublishLimiter(inboundPublishLogInterval)
	return c
}
...
var errInboundPublishQueueOverflow = errors.New("inbound publish queue overflow")
...
func (c *client) startCommsWorkers(conn net.Conn, connectionUp connCompletedFn, inboundFromStore <-chan packets.ControlPacket) bool {
	...
	queueDepth := int(c.options.InboundPublishQueueDepth)
	if queueDepth < 0 {
		queueDepth = 0
	}
	incomingPubChan := make(chan *packets.PublishPacket, queueDepth)
	c.inboundPublishQueueMu.Lock()
	c.inboundPublishQueue = incomingPubChan
	c.inboundPublishQueueMu.Unlock()
	workerCount := c.options.InboundPublishWorkers
	if workerCount <= 0 {
		workerCount = 1
	}
	c.workers.Add(1) // Done will be called when ackOut is closed
	ackOut := c.msgRouter.matchAndDispatch(incomingPubChan, c.options.Order, workerCount, c)
	...
	go func() {
		...
		enqueueTimeout := c.options.InboundPublishEnqueueTimeout
		defer func() {
			if enqueueTimer != nil {
				enqueueTimer.Stop()
			}
		...
		for {
			...
			case pub, ok := <-commsIncomingPub:
				if !ok {
					...
					c.inboundPublishQueueMu.Lock()
					c.inboundPublishQueue = nil
					c.inboundPublishQueueMu.Unlock()
					commsIncomingPub = nil
					continue
				}
				result, err := enqueueInboundPublish(incomingPubChan, queueDepth, pub, commsErrors, enqueueTimeout, &enqueueTimer)
				switch result {
				case inboundEnqueueOK:
					// nothing to do
				case inboundEnqueueDroppedQoS0:
					dropped := atomic.AddUint64(&c.inboundPublishDropsQoS0, 1)
					if c.inboundPublishDropLimiter.Allow(time.Now()) {
						WARN.Printf("%sInbound publish queue full; dropping QoS0 publish (dropped=%d queue=%d/%d)", CLI, dropped, len(incomingPubChan), cap(incomingPubChan))
					}
				case inboundEnqueueTimeoutQoS12:
					timeouts := atomic.AddUint64(&c.inboundPublishQoS12Timeouts, 1)
					disconnects := atomic.AddUint64(&c.inboundPublishQoS12Disconnects, 1)
					if c.inboundPublishQoS12Limiter.Allow(time.Now()) {
						if enqueueTimeout > 0 {
							WARN.Printf("%sInbound publish queue full; QoS%d enqueue timed out after %s (timeouts=%d disconnects=%d queue=%d/%d)", CLI, pub.Qos, enqueueTimeout, timeouts, disconnects, len(incomingPubChan), cap(incomingPubChan))
						} else {
							WARN.Printf("%sInbound publish queue full; QoS%d enqueue timed out (timeouts=%d disconnects=%d queue=%d/%d)", CLI, pub.Qos, timeouts, disconnects, len(incomingPubChan), cap(incomingPubChan))
						}
					}
					c.internalConnLost(errInboundPublishQueueOverflow)
				case inboundEnqueueCommsError:
					if err != nil {
