...
// matchAndDispatch takes a channel of Message pointers as input and starts a bounded worker pool
// that matches messages against the internal route list and calls the associated callback
// (or the defaultHandler, if one exists and no other route matched). If order is true, a single
// worker is used to preserve ordering.
func (r *router) matchAndDispatch(messages <-chan *packets.PublishPacket, order bool, workerCount int, client *client) <-chan *PacketAndToken {
	ackChan := make(chan *PacketAndToken) // Channel returned to caller; closed when workers terminate
	...
	if order || workerCount <= 0 {
		workerCount = 1
	}
	var wg sync.WaitGroup
	wg.Add(workerCount)
	for i := 0; i < workerCount; i++ {
		go func() { // Worker goroutine handling inbound messages.
			defer wg.Done()
			handlers := make([]MessageHandler, 0, 4)
