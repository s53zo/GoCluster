...
func main() {
	...
	pathCfg, err := pathreliability.LoadFile(*pathConfigFlag)
	if err != nil {
		log.Printf("Warning: failed to load path reliability config (%s): %v", *pathConfigFlag, err)
	...
	must(os.MkdirAll(filepath.Dir(jsonOut), 0o755))
	must(os.WriteFile(jsonOut, jsonBytes, 0o644))

	finalReport := buildFinalReport(summary)
	if !*noLLMFlag {
		openaiCfg, err := loadOpenAIConfig(*openAIConfigFlag)
		if err != nil {
			log.Printf("Warning: failed to load OpenAI config (%s): %v", *openAIConfigFlag, err)
		} else {
			ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
			defer cancel()
			llmText, err := openaiutil.Generate(ctx, openaiutil.Config{
			...
			if err != nil {
				log.Printf("Warning: OpenAI request failed: %v", err)
	...
	must(os.MkdirAll(filepath.Dir(reportOut), 0o755))
	must(os.WriteFile(reportOut, []byte(finalReport+"\n"), 0o644))
