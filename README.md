...
- Maintains a single directional FT8-equivalent bucket family per path: FT8/FT4/CW/RTTY/PSK all feed the same buckets. Voice modes (LSB/USB) are display-only. Buckets use a 2x2 grid per Maidenhead field plus coarse grid2, per-band half-lives, and staleness purging (per-band stale window).
