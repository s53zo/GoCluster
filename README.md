...
- Maintains a single directional FT8-equivalent bucket family per path: FT8/FT4/CW/RTTY/PSK all feed the same buckets. Voice modes (LSB/USB) are display-only. Buckets use a 2x2 grid per Maidenhead field plus coarse grid2, per-band half-lives, and staleness purging.
...
- System log: every 5 minutes emits `Path predictions (5m)` (combined vs insufficient), `Path buckets (5m)` (per-band bucket counts), and `Path weight dist (5m)` (per-band combined weight histogram).
